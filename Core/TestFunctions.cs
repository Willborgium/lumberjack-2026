...
public static class TestFunctions
{
    ...
    public static Renderable3D<VertexPositionNormalTextureColor> CreateTexturedCube(Effect effect, Texture2D texture, float size = 1f)
    {
        return CreateTexturedRectangularPrism(effect, texture, size, size, size);
    }

    public static Renderable3D<VertexPositionNormalTextureColor> CreateTexturedSphere(Effect effect, Texture2D texture, int stacks = 8, int slices = 12, float radius = 1f)
    {
        var positions = new List<Vector3>();
        ...
        return BuildTexturedRenderable(effect, texture, [.. positions], inds.ToArray(), [.. uvs]);
    }

    public static Renderable3D<VertexPositionNormalTextureColor> CreateTexturedPyramid(Effect effect, Texture2D texture, float size = 1f, float height = 1.2f)
    {
        float s = size;
        var positions = new Vector3[5];
        positions[0] = new Vector3(-s, 0, -s);
        ...
        var uvs = ComputePlanarUvs(positions);
        return BuildTexturedRenderable(effect, texture, positions, idx, uvs);
    }

    public static Renderable3D<VertexPositionNormalTextureColor> CreateTexturedRectangularPrism(Effect effect, Texture2D texture, float width = 1.5f, float height = 0.8f, float depth = 0.6f)
    {
        float hw = width * 0.5f;
    ...
    private static Renderable3D<VertexPositionNormalTextureColor> BuildTexturedRenderable(Effect effect, Texture2D texture, Vector3[] positions, short[] indices, Vector2[] uvs)
    {
        var normals = ComputeNormals(positions, indices);
        var verts = new VertexPositionNormalTextureColor[positions.Length];
        for (int i = 0; i < positions.Length; i++)
        {
            verts[i] = new VertexPositionNormalTextureColor(positions[i], normals[i], uvs[i], Color.White);
        }

        return new Renderable3D<VertexPositionNormalTextureColor>(effect, verts, indices, texture);
    }

    private static Vector2[] ComputePlanarUvs(Vector3[] positions)
    {
        ...
        var rangeX = MathF.Max(maxX - minX, 0.000001f);
        var rangeZ = MathF.Max(maxZ - minZ, 0.000001f);

        var uvs = new Vector2[positions.Length];
        for (int i = 0; i < positions.Length; i++)
        {
            var p = positions[i];
            uvs[i] = new Vector2((p.X - minX) / rangeX, (p.Z - minZ) / rangeZ);
        }
